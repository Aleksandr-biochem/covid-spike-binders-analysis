...
trajectory_stride = 17500
script_output_folder = 'analysis_output_deltaplus_pdb'
...
def apply_zemu_gam(scores):
    ...
    new_columns.remove(('total_score', 'mean'))
    scores = scores.copy()[ new_columns ]
    for score_term in zemu_gam_params:
        assert( (score_term, "mean") in scores.columns )
        scores[(score_term, "mean")] = scores[(score_term, "mean")].apply( gam_function, score_term = score_term)
    scores[( 'total_score', 'mean') ] = scores[ list(zip(list(zemu_gam_params.keys()), ["mean"]*7 ))].sum( axis = 1 )
...
def find_finished_jobs( output_folder ):
    ...
    #job_dir = dir with the mutation:  abspath
    #struct_dirs - 1, 2, 3, ... : abspath
...
def get_scores_from_db3_file(db3_file, struct_number, case_name):
    ...
    # we get new table: 1) column name (batch_id) batch_id: 0 - bound_wt, 1 - unbound_wt, 2 - bound_mut, 3 - unbound-mut
    # 2) column struct_id =backrub_steps . We will renumber it. Result depends on the trajectory stride. If stride == 5, then: 5, 5, 5, 5, 10, 10, 10, 10, 15, 15, 15, 15, 20...  etc.
    # 3) score_type_name (for example fa_atr, fa_rep etc.)
    # 4) score_value
    # 5) score_function_name (for example fa_talaris_2014). There are a lot of (46) options pf this function. Not sure that we use them,
    # but in the resulted table every row is repeated 46 times
    ...
    #resulted table: columns - score_type_names, rows - backrub steps and states
...
def process_finished_struct( output_path, case_name ):
    # output_path: abspath of the struct: /home/olga/cov2/Rosetta3/delta_plus/output_saturation_deltaplus1200/deltaplus1200_B222_E/03/
    #case_name: basename of the job: deltaplus1200_B222_E
    #output_database_name: 'ddG.db3'
...
def calc_ddg( scores ):
    ...
    for nstructs in nstructs_to_analyze:
        # these states we'll multiply by -1
        ...
        ddg_scores = ddg_scores.groupby( ['case_name', 'backrub_steps', 'score_function_name'] ).agg(['mean', 'std']).round(decimals=5).reset_index()
        new_columns = list(ddg_scores.columns.values)
        new_columns.remove( ('struct_num', 'mean') )
        new_columns.remove( ('struct_num', 'std') )
...
def calc_dgs( scores ):
    ...
    for state in ['mut', 'wt']:
        for nstructs in nstructs_to_analyze:
            ...
            dg_scores = dg_scores.groupby( ['case_name', 'backrub_steps', 'score_function_name'] ).agg(['mean', 'std']).round(decimals=5).reset_index()
...
def analyze_output_folder( output_folder ):
    ...
    #output_folder = output_saturation
    ...
    ddg_scores_dfs = []
    struct_scores_dfs = []
    #finished_job 1JTC, finished_structs - 1, 2, 3
    for finished_job, finished_structs in finished_jobs.items():
        #finished_job - for example deltaplus1200_B222_A
        #finished_structs - 1, 2, 3, 4, 5....
        inner_scores_list = []
        for finished_struct in finished_structs:
            # abspath of the struct: /home/olga/cov2/Rosetta3/delta_plus/output_saturation_deltaplus1200/deltaplus1200_B222_E/03/
            #basename of the job: deltaplus1200_B222_E
            inner_scores = process_finished_struct( finished_struct, os.path.basename(finished_job) )
            # inner_scores - pivot table
            inner_scores_list.append( inner_scores )
        scores = pd.concat( inner_scores_list )
        #get one table with the same number of columns, but a lot of rows
