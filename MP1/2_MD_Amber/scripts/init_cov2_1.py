...
"""
        ...
        ## minimize environment (except protein structure - to avoid overlaps)
        ...
        # heat from 0 to target Kelvin temperature (~25 C), speed - Maxwell speed of molecules; each step - reassign of T and speed
        md.heat.input.cntrl( 
            imin=0,
            irest=0,
            ntx=1,
        ...
        # have to recallibrate kinetically, as previously potential E was not increased (equeal P and K energies)
        ...
        md.production.input.cntrl(
            ...
            ntwr=500000,  # md restart frequency
            ioutfm=1,
            ntwprt=num_atoms # save set number of atoms - decrease size of file (exclude water)
        )
...
class variant(MdProtocol):
    def __init__(self, path_to_initial_dir, name):
...
if __name__ == '__main__':
    ...
    path_to_initial_dir = "../sample"
    for name in ["wt+mp1", "alpha+mp1", "delta+mp1", "delta_plus+mp1", "omicron+mp1"]:
        md = variant(path_to_initial_dir, name)
        #os.makedirs(name, exist_ok=True)
        md.save(Path(name + "/state.dill"))
