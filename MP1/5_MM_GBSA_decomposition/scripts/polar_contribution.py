...
def polar_contribution_data(path_to_trj=".", outpth=".", lcb_residue="", rbd_residue=""):
	"""
    ...
    param: lcb_residue - LCB residue of interest, e.g. SER29
	...
	for mut in mutants: 
		...
		for residue_pair in tqdm(complex_residue_pairs, desc=f"igb8_model {mut}"):
			...
			if (rid_lcb == int(lcb_residue[3:])) and (rid_rbd == int(rbd_residue[3:])):
				...
				continue
		outd_path = outpth
		out_polar_contr.to_csv(os.path.join(outd_path, f"{mut}+mp1_polar_{lcb_residue}-{rbd_residue}.tsv"), sep='\t', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r1', nargs=1, help='LCB residue in the format AA№, where AA - three letter amino acid code', type=str, required=True)
    ...
    lcb = str(args.r1[0])
    rbd = str(args.r2[0])
    polar_contribution_data(path_to_trj=args.i[0], outpth=args.o[0], lcb_residue=lcb, rbd_residue=rbd)
