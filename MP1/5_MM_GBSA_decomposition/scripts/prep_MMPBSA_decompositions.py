...
class PrepareMMPBSA(remote_runner.Task):
    def __init__(self, path_to_complex_trj_dir,
        ...
        self.mmpbsa_dir = os.path.join(path_to_complex_trj_dir, "8_mmpbsa_decomposition")
    ...
    def change(self, igb_option):
        ...
        self.input_parameters = f"igb{igb_option}_{self.salt}_decomp.in"
...
if __name__ == '__main__':
    ...
    for complexe in [
        ...
        task = PrepareMMPBSA(path_to_complex_trj_dir=os.path.join(prefix, complexe),
                             traj_length=1499)

        task.change(igb_option=2)
        task.save(task.wd / 'state.dill')

        task.change(igb_option=8)
        task.save(task.wd / 'state.dill')
